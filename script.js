...
window.addEventListener('scroll',()=>{const h=document.documentElement;pb.style.width=(h.scrollTop/(h.scrollHeight-h.clientHeight))*100+'%';smoke.style.transform=`translateY(${window.scrollY*0.18}px)`;document.querySelector('.hero').style.backgroundPositionY=`${window.scrollY*0.12}px`;});
